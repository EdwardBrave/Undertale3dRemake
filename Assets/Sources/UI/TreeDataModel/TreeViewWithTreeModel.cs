...
namespace UI.TreeDataModel
{
	...
	public class TreeViewWithTreeModel<T> : TreeView where T : TreeElement, new()
	{
		...
		protected override TreeViewItem BuildRoot()
		{
			...
			return new TreeViewItem<T>(_treeModel.root.id, depthForHiddenRoot, _treeModel.root.name,
				_treeModel.root);
		}
		...
		public void DrawToolBar(MonoBehaviour asset)
		{
			using (new EditorGUILayout.HorizontalScope())
			{
				...
				if (GUILayout.Button("Add Item", style))
				{
					...
					TreeElement parent;
					if (isRecursive)
						parent = (selection.Count == 1 ? treeModel.Find(selection[0]) : null) ?? treeModel.root;
					else
						parent = treeModel.root;
					int depth = parent?.depth + 1 ?? 0;
		...
		public void DrawSearchBar(Rect baseRect)
		{
			if (_searchField == null)
			{
				_searchField = new SearchField();
				_searchField.downOrUpArrowKeyPressed += SetFocusAndEnsureSelectedItem;
			}

			searchString = _searchField.OnGUI(baseRect, searchString);
		}
