...
namespace UnityEngine.Events
{
	...
	/// <summary>
	/// Events 2.0 for Unity with six generic types
	/// </summary>
	[System.Serializable]
	public abstract class UnityEvent2<T0, T1, T2, T3, T4, T5> : UnityEventBase2
	{
		...
		{
			return GetValidMethodInfo(targetObj, name, new System.Type[6] { typeof(T0), typeof(T1), typeof(T2), typeof(T3), typeof(T4), typeof(T5) });
		}
		...
		private static BaseInvokableCall2 GetDelegate(UnityAction<T0, T1, T2, T3, T4, T5> action)
		{
			return new InvokableCall2<T0, T1, T2, T3, T4, T5>(action);
		}
		...
		public void Invoke(T0 arg1, T1 arg2, T2 arg3, T3 arg4, T4 arg5, T5 arg6)
		{
			...
			{
				var curCall = calls[i] as InvokableCall2<T0, T1, T2, T3, T4, T5>;
				if (curCall != null)
		...
		internal void AddPersistentListener(UnityAction<T0, T1, T2, T3, T4, T5> call, UnityEventCallState callState)
		{
