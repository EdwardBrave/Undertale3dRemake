...
namespace UnityEngine.UI
{
	...
	[AddComponentMenu("UI/Input Field 2.0", 32)]
	public class InputField2
		: Selectable,
		...
		ILayoutElement
	{
		...
		[Serializable]
		public class SubmitEvent2 : UnityEvent2<string> { }

		[Serializable]
		public class OnChangeEvent2 : UnityEvent2<string> { }
		...
		[SerializeField]
		[Tooltip("Events to trigger when user is done editing the field")]
		private SubmitEvent2 m_OnEndEdit = new SubmitEvent2();
		...
		[SerializeField]
		[Tooltip("Events to trigger when this input field value is changed")]
		private OnChangeEvent2 m_OnValueChanged = new OnChangeEvent2();
		...
		public bool shouldHideMobileInput
		{
			set
			{
				SetPropertyUtility2.SetStruct(ref m_HideMobileInput, value);
			}
			get
			{
		...
		public string text
		{
			...
			set
			{
				...
				if (m_LineType == InputField.LineType.SingleLine)
					value = value.Replace("\n", "").Replace("\t", "");

				// If we have an input validator, validate the input and apply the character limit at the same time.
				if (onValidateInput != null || characterValidation != InputField.CharacterValidation.None)
				{
					m_Text = "";
		...
		public float caretBlinkRate
		{
			...
			set
			{
				if (SetPropertyUtility2.SetStruct(ref m_CaretBlinkRate, value))
				{
					if (m_AllowInput)
						SetCaretActive();
				}
		...
		public int caretWidth { get { return m_CaretWidth; } set { if (SetPropertyUtility2.SetStruct(ref m_CaretWidth, value)) MarkGeometryAsDirty(); } }
		...
		public Text textComponent
		{
			...
			set
			{
				...
				if (SetPropertyUtility2.SetClass(ref m_TextComponent, value))
				{
					EnforceTextHOverflow();
					if (m_TextComponent != null)
					{
		...
		public Graphic placeholder { get { return m_Placeholder; } set { SetPropertyUtility2.SetClass(ref m_Placeholder, value); } }

		public Color caretColor { get { return customCaretColor ? m_CaretColor : textComponent.color; } set { if (SetPropertyUtility2.SetColor(ref m_CaretColor, value)) MarkGeometryAsDirty(); } }
		...
		public Color selectionColor { get { return m_SelectionColor; } set { if (SetPropertyUtility2.SetColor(ref m_SelectionColor, value)) MarkGeometryAsDirty(); } }

		public SubmitEvent2 onEndEdit { get { return m_OnEndEdit; } set { SetPropertyUtility2.SetClass(ref m_OnEndEdit, value); } }

		[Obsolete("onValueChange has been renamed to onValueChanged")]
		public OnChangeEvent2 onValueChange { get { return onValueChanged; } set { onValueChanged = value; } }

		public OnChangeEvent2 onValueChanged { get { return m_OnValueChanged; } set { SetPropertyUtility2.SetClass(ref m_OnValueChanged, value); } }

		public OnValidateInput onValidateInput { get { return m_OnValidateInput; } set { SetPropertyUtility2.SetClass(ref m_OnValidateInput, value); } }

		public int characterLimit { get { return m_CharacterLimit; } set { if (SetPropertyUtility2.SetStruct(ref m_CharacterLimit, Math.Max(0, value))) UpdateLabel(); } }

		// Content Type related

		public InputField.ContentType contentType { get { return m_ContentType; } set { if (SetPropertyUtility2.SetStruct(ref m_ContentType, value)) EnforceContentType(); } }
		...
		public InputField.LineType lineType
		{
			...
			set
			{
				if (SetPropertyUtility2.SetStruct(ref m_LineType, value))
				{
		...
		public InputField.InputType inputType { get { return m_InputType; } set { if (SetPropertyUtility2.SetStruct(ref m_InputType, value)) SetToCustom(); } }
...
#endif
#endif
				if (SetPropertyUtility2.SetStruct(ref m_KeyboardType, value))
					SetToCustom();
			}
		}

		public InputField.CharacterValidation characterValidation { get { return m_CharacterValidation; } set { if (SetPropertyUtility2.SetStruct(ref m_CharacterValidation, value)) SetToCustom(); } }

		public bool readOnly { get { return m_ReadOnly; } set { m_ReadOnly = value; } }

		// Derived property
		public bool multiLine { get { return m_LineType == InputField.LineType.MultiLineNewline || lineType == InputField.LineType.MultiLineSubmit; } }
		// Not shown in Inspector.
		public char asteriskChar { get { return m_AsteriskChar; } set { if (SetPropertyUtility2.SetStruct(ref m_AsteriskChar, value)) UpdateLabel(); } }
			...
			if (m_Text != val)
			{
				...
				else
				{
					...
					for (int i = 0; i < val.Length; ++i)
					{
						...
						else if (characterValidation != InputField.CharacterValidation.None)
							c = Validate(m_Text, m_Text.Length, c);

						if (lineType == InputField.LineType.MultiLineSubmit && c == '\n')
						{
							m_Keyboard.text = m_Text;
		...
		protected char Validate(string text, int pos, char ch)
		{
			...
			if (characterValidation == InputField.CharacterValidation.None || !enabled)
				return ch;

			if (characterValidation == InputField.CharacterValidation.Integer || characterValidation == InputField.CharacterValidation.Decimal)
			{
				// Integer and decimal
				...
				if (!cursorBeforeDash || dashInSelection)
				{
					...
					if (ch == '.' && characterValidation == InputField.CharacterValidation.Decimal && !text.Contains(".")) return ch;
				}
			}
			else if (characterValidation == InputField.CharacterValidation.Alphanumeric)
			{
			...
			else if (characterValidation == InputField.CharacterValidation.Name)
			{
			...
			else if (characterValidation == InputField.CharacterValidation.EmailAddress)
			{
		...
		private void ActivateInputFieldInternal()
		{
			...
			if (TouchScreenKeyboard.isSupported)
			{
				...
				m_Keyboard = (inputType == InputField.InputType.Password) ?
					TouchScreenKeyboard.Open(m_Text, keyboardType, false, multiLine, true) :
					TouchScreenKeyboard.Open(m_Text, keyboardType, inputType == InputField.InputType.AutoCorrect, multiLine);
