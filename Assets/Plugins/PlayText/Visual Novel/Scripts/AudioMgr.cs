...
namespace FrameWork
{
    public class AudioMgr : SingleClassBase<AudioMgr>
    {
        ...
        public void PlayBackMusic(string name, float v = 1, bool FadeIn = false, float FadeInTime = 3, bool FadeOut = false, float FadeOutTime = 3)
        {
            if (BGMusic == null)
            {
                GameObject obj = new GameObject();
            ...
            StopBackMusic(FadeOut, FadeOutTime);

            //异步加载背景音乐并播放
            ResMgr.GetInstance().LoadAsync<AudioClip>(name, (Clip) => {
                StartCoroutine(WaitForPlay(Clip, v, FadeIn, FadeInTime));
            });
        }

        IEnumerator WaitForPlay(AudioClip Clip, float v, bool FadeIn, float FadeInTime)
        {
            while (Stopping)
                yield return null;
            ...
            BGMusic.volume = 0;
            if (FadeIn)
                BGMusic.DOFade(BGMVolume, FadeInTime);
            else
                BGMusic.volume = BGMVolume;
            BGMusic.loop = true;
            BGMusic.Play();
        ...
        /// 停止背景音乐
        /// </summary>
        public void StopBackMusic(bool FadeOut = false, float FadeOutTime = 3)
        {
            if (BGMusic == null)
                return;
            ...
            if (FadeOut)
            {
                StartCoroutine(StopBGM(FadeOutTime));
            }
            else
            {
                BGMusic.Stop();
        ...
        IEnumerator StopBGM(float FadeOutTime)
        {
            Stopping = true;
            BGMusic.DOFade(0, FadeOutTime);
